...
PyObject* K_CONNECTOR::___setInterpTransfers(PyObject* self, PyObject* args)
{
  ...
  ipt_roR            = new E_Float*[nidomR*11]; //1
  ...
  ipt_ro_tijModel    = ipt_psiGR_vert  + nidomR;//11
...
# pragma omp parallel default(shared)
  {
    ...
    vector<E_Float*> vectOfDnrFields(nvars);
    vector<E_Float*> vectFieldsTijModel(nvarsWMLESLin);
			...
			if (nvars_loc == 5 || nvars_loc == 6) // Transferts NS classiques ou LBM -> NS
			  {
			    ...
			    if (isWMLESLin==1)
			      {
				for (E_Int eq = 0; eq < nvarsWMLESLin; eq++){
				  vectFieldsTijModel[eq] = ipt_ro_tijModel[ NoR] + eq*ipt_param_intR[ NoR ][ NDIMDX ];
				}
			      }
			  }
				...
				if (isWMLESLin==1)
				  {
					...
					vectFieldsTijModel[0][indR]=(densPtr+nbRcvPts*7 )[noind+pt_deb];  //t11
					vectFieldsTijModel[1][indR]=(densPtr+nbRcvPts*8 )[noind+pt_deb];  //t12
					vectFieldsTijModel[2][indR]=(densPtr+nbRcvPts*9 )[noind+pt_deb];  //t22
					vectFieldsTijModel[3][indR]=(densPtr+nbRcvPts*10 )[noind+pt_deb]; //t13
					vectFieldsTijModel[4][indR]=(densPtr+nbRcvPts*11 )[noind+pt_deb]; //t23
					vectFieldsTijModel[5][indR]=(densPtr+nbRcvPts*12 )[noind+pt_deb]; //t33
				      }
				  }
